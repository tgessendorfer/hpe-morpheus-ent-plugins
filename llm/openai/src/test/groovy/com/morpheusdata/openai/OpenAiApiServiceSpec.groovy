 ...
 * on 2026-09-14.
 */
class OpenAiApiServiceSpec extends Specification {

	OpenAiApiService service = new OpenAiApiService()
	HttpServer server
	...
	def "ASCII-only JSON keeps emoji and umlauts decodable"() {
		expect:
		OpenAiApiService.toAsciiJson([text: 'Läuft 👋']) ==~ /[\x00-\x7E]*/
		new JsonSlurper().parseText(OpenAiApiService.toAsciiJson([text: 'Läuft 👋'])).text == 'Läuft 👋'
	}
	...
	def "an error event after the stream started aborts it"() {
		...
		then:
		OpenAiApiService.StreamErrorException e = thrown()
	...
	def "a refused stream reports its status and how long the API asks to wait"() {
		given:
		String baseUrl = serve('/chat/completions', 'application/json',
			'{"error":{"message":"Rate limit exceeded","code":429},"user_id":"user_abc123"}', 429, [], ['Retry-After': '7'])
	...
	def "an error event inside a stream comes back as a failure with its status"() {
		...
		result.msg == 'Stream error: Provider returned error'
	}

	def "status codes and Retry-After values are read defensively"() {
		expect:
		OpenAiApiService.statusCodeOf(status) == expectedStatus
		OpenAiApiService.retryAfterSeconds(retryAfter) == expectedWait
	...
	def "the api service applies the proxy to the client and clears it again"() {
		...
		when:
		service.applyNetworkProxy(client, [(OpenAiApiService.NETWORK_PROXY_KEY): proxy])
