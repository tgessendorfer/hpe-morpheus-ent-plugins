...
/**
 * HTTP client service for an OpenAI-compatible API.
 *
 *  - authentication with an {@code Authorization: Bearer} header, when there is a key
 *  - POST /chat/completions for both streaming and non-streaming completions
 *  - GET /models for the catalog
 *
 * HttpApiClient appends a path to the base URL's own path verbatim, so the base
 * URL carries the API prefix: https://api.openai.com/v1.
 */
@Slf4j
class OpenAiApiService {
	...
	// OpenRouter only: the record of the calling key, 401 to an unknown one, free.
	...
	/** An error the API sent inside a stream that had already started, with its code. */
	...
	/**
	 * The model catalog. Most APIs answer it only to a valid key, so it doubles as
	 * the credential check on integration save; OpenRouter serves it to anyone.
	 */
	...
	/**
	 * OpenRouter's key record. Its model list is public, so this is the only free
	 * way to find out on save whether the key is any good.
	 */
		...
		try {
			...
			return withApiClient(opts ?: [:]) { HttpApiClient apiClient ->
				...
				if (apiResponse?.success != true || response == null) {
					...
					return [success   : false, msg: "API returned ${statusCode}: ${detail}".toString(),
			...
			log.warn("Stream ended with an error: ${e.message}")
			return [success: false, msg: e.message, statusCode: e.statusCode]
		} catch (Exception e) {
			log.error("Error during streaming completion: ${e.message}", e)
	 ...
	 *
	 * What OpenRouter and api.openai.com send, as observed:
		...
		response.entity?.content?.withReader('UTF-8') { Reader reader ->
			...
			while ((line = reader.readLine()) != null) {
				...
				if (event.error instanceof Map) {
					...
					throw new StreamErrorException("Stream error: ${describeError(error)}".toString(), statusCodeOf(error.code))
				}
	 ...
	 * no SNI in the TLS handshake, which Cloudflare-fronted endpoints such as
	 * openrouter.ai answer with handshake_failure (still so on Morpheus 9.0.2). That
	 * is why HPE's Local LLM plugin cannot reach them, and why this plugin offers no
	 * "ignore certificate errors" option: it would trade one failure for another.
	 */
	protected HttpApiClient.RequestOptions buildRequestOptions(String apiKey, Map<CharSequence, CharSequence> headers, Object body, Integer readTimeout) {
		...
		if (body != null) {
			...
			// ISO-8859-1, and the API turned every umlaut into U+FFFD. With every
	...
	/**
	 * Errors look like {"error":{"message":"...","type":"...","code":...}} at every
	 * OpenAI-compatible API; OpenRouter adds "metadata" and a top-level "user_id".
	 * Only the message is used: an account id has no business in the appliance log.
	 */
	...
	/** The message of an error object, with the upstream provider when OpenRouter names one. */
