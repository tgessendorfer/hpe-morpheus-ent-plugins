...
class LlmModelsSync {
	...
	LlmModelsSync(MorpheusContext morpheusContext, LlmIntegration llmIntegration, String providerCode, OpenRouterApiService apiService) {
		this.morpheusContext = morpheusContext
		this.llmIntegration = llmIntegration
		this.providerCode = providerCode
		this.apiService = apiService ?: new OpenRouterApiService()
	}

	Map execute(String baseUrl, String apiKey, Map opts = [:], Closure<Collection<LlmModel>> modelBuilder) {
		Map result = apiService.listModels(baseUrl, apiKey, opts) ?: [success: false, msg: 'No response from the OpenRouter models API']
		if (!result.success) {
			log.warn("Failed to refresh models from OpenRouter: ${result.msg}")
			return result
		}
	...
	void execute(Collection<LlmModel> freshModels) {
		if (!morpheusContext || !providerCode) {
			log.warn('Skipping OpenRouter model sync: sync context is incomplete')
			return
		}
		if (!llmIntegration?.id) {
			log.warn('Skipping OpenRouter model sync: LLM integration is not persisted')
			return
		}
